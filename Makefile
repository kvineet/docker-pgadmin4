...
test-nolink: 
	docker run -it -d --name pgadmin-dev -p 5433:80 -v `pwd`/.pgadmin4:/data kvineet/docker-pgadmin4:dev
build:
...
run-nolink:
	docker run -it -d --name pgadmin -p 5433:80 -v `pwd`/.pgadmin4:/data kvineet/docker-pgadmin4:latest
